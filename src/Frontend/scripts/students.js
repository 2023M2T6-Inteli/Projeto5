...
// Obtém o ID da classe da URL
const id = getClassIdFromUrl();

// Função para buscar os alunos pelo ID da classe
...
// Função para redirecionar para a página de registro de aluno
...
// Função para filtrar os alunos por nome
...
// Função para buscar as médias das notas da classe
async function fetchAverageClassGrades(classId) {
  try {
    const response = await fetch(`http://localhost:3000/studentGrades/student/${classId}`);
    if (!response.ok) {
      throw new Error(`Request failed with status ${response.status}`);
    }
    const data = await response.json();
...
// Função para criar as divs dos alunos
function createDivs(students) {
  const container = document.querySelector('#students-container');

  students.forEach(async (studentData, studentIndex) => {
    const { id, class_id, name, call_number } = studentData;
    try {
...
// Função para buscar as notas aleatórias do Algolia
